...
function run_kohonen($type_cluster,$cl_useold) {
	global $CONFIG;
	...
	$kohonen_c = $CONFIG->path . "mod/profile_manager/views/default/profile_manager/members/kohonen";
	$doc_term = '';
	...
	for($i = 0 ; $i < count($array_guid); $i++){
		if(count($array_cluster[$info_cluster[$i] ]) == ''){
			$array_cluster[$info_cluster[$i] ] = new Cluster($type_cluster,get_loggedin_user()->username,"kohonen",1);
		}
