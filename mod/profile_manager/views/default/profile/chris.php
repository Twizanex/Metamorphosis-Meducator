<div id='profile_manager_profile_edit_tab_content_1969' class='profile_manager_profile_edit_tab_content'><!-- form from Chris Valentine -->
  <style type="text/css">
    .noBox { border: 0; margin-bottom: 4px; font-size: 90%; background-color: #ffffff; }
    .responseSelector { font-size: 85%; border: 1px solid #000000; margin: 4px 0; width: 500px; }
    .mainSelectors { font-size: 85%; border: 1px solid #000000; margin: 4px 0; width: 330px; }
    .dim { color: #101010; }
    #bioPortal_1 { width: 500px; float: left; }
    .control_buttons_BP { width: 150px; text-align: center; float: left; padding-top: 50px; }
    .control_buttons_BP input { margin-bottom: 10px; }
    .control_buttons_disabled { color: #cccccc; }
    #bioPortal_2 { width: 400px; float: left; }
    .chosen_boxes { float: left; width: 33%; margin-top: 50px; }
  </style>
  <script type="text/javascript">/* <![CDATA[ */
    ...
    //make all necessary settings for selecting specialty
    function addDisCompute(form, val, text, label, concept)
    {
      select=form.pickedDis;
      l=select.length;
      val = pickedDisLabels.length;
      select.options[l]=new Option(text + " | " + label, val);
      pickedDisIndex[val]=val;
      pickedDisLabels[val]=label;
      pickedDisIds[val]=concept;
      pickedDisNames[val]=text;
    }
    
    // pick a hit as a discipline
    function addDisHit(form) {
      //check if we have any ontology results
      if(labels.length == 0) //we don't'
        return addDisNoHit(form);
      
      //we have results from ontology
      ...
      addDisCompute(form, newVal, preferredNames[newVal], labels[newVal], conceptIds[newVal]);
      return true;
    }
    
    function addDisNoHit(form)
    {
      var auxLabel = $('#searchfor').val();
      var ontURI = rstr2hex(auxLabel);
      addDisCompute(form, ontURI, auxLabel, "Free text", ontURI);
      return true;
    }
    
    //make all necessary settings for selecting specialty
    function addKeyCompute(form, val, text, label, concept)
    {
      select=form.pickedKey;
      l=select.length;
      val = pickedKeyLabels.length;
      select.options[l]=new Option(text + " | " + label, val);
      pickedKeyIndex[val]=val;
      pickedKeyLabels[val]=label;
      pickedKeyIds[val]=concept;
      pickedKeyNames[val]=text;
    }
    ...
    function addKeyHit(form) {
      ...
      addKeyCompute(form, newVal, preferredNames[newVal], labels[newVal], conceptIds[newVal]);
      return true;
    }
    
    function addKeyNoHit(form)
    {
      var auxLabel = $('#searchfor').val();
      var ontURI = rstr2hex(auxLabel);
      addKeyCompute(form, ontURI, auxLabel, "Free text", ontURI);
      return true;
    }
    
    //make all necessary settings for selecting specialty
    function addSpecCompute(form, val, text, label, concept)
    {
      select=form.pickedSpec;
      l=select.length;
      val = pickedSpecLabels.length;
      select.options[l]=new Option(text + " | " + label, val);
      pickedSpecIndex[val]=val;
      pickedSpecLabels[val]=label;
      pickedSpecIds[val]=concept;
      pickedSpecNames[val]=text;
    }
    ...
    function addSpecHit(form) {
      ...
      addSpecCompute(form, newVal, preferredNames[newVal], labels[newVal], conceptIds[newVal]);
      return true;
    }
    
    function addSpecNoHit(form)
    {
      var auxLabel = $('#searchfor').val();
      var ontURI = rstr2hex(auxLabel);
      addSpecCompute(form, ontURI, auxLabel, "Free text", ontURI);
      return true;
    }
    ...
    function buildRDF() 
    {
      ...
      for (i=0; i < pickedDisLabels.length; i++) {
        if (pickedDisLabels[i]) {
          if(DisLabelsList != '') DisLabelsList += '|';
          DisLabelsList += pickedDisLabels[i];
      ...
      for (i=0; i < pickedDisIds.length; i++) {
        if (pickedDisIds[i]) {
          if(DisIdsList != '') DisIdsList += '|';
          DisIdsList += pickedDisIds[i];
      ...
      for (i=0; i < pickedDisNames.length; i++) {
        if (pickedDisNames[i]) {
          if(DisNamesList != '') DisNamesList += '|';
          DisNamesList += pickedDisNames[i];
      ...
      for (i=0; i < pickedKeyLabels.length; i++) {
        if (pickedKeyLabels[i]) {
          if(KeyLabelsList != "") KeyLabelsList += '|';
          KeyLabelsList += pickedKeyLabels[i];
      ...
      for (i=0; i < pickedKeyIds.length; i++) {
        if (pickedKeyIds[i]) {
          if(KeyIdsList != '') KeyIdsList += '|';
          KeyIdsList += pickedKeyIds[i];
      ...
      for (i=0; i < pickedKeyNames.length; i++) {
        if (pickedKeyNames[i]) {
          if(KeyNamesList != '') KeyNamesList += '|';
          KeyNamesList += pickedKeyNames[i];
      ...
      for (i=0; i < pickedSpecLabels.length; i++) {
        if (pickedSpecLabels[i]) {
          if(SpecLabelsList != '') SpecLabelsList += '|';
          SpecLabelsList += pickedSpecLabels[i];
      ...
      for (i=0; i < pickedSpecIds.length; i++) {
        if (pickedSpecIds[i]) {
          if(SpecIdsList != '') SpecIdsList += '|';
          SpecIdsList += pickedSpecIds[i];
      ...
      for (i=0; i < pickedSpecNames.length; i++) {
        if (pickedSpecNames[i]) {
          if(SpecNamesList != '') SpecNamesList += '|';
          SpecNamesList += pickedSpecNames[i];
    ...
    function fetchHits(form) 
    {
	  ...
	  document.getElementById('wait').src='/mod/profile_manager/views/default/profile/wait_animated.gif';
      //added by Teodor -> clear previous results
      form.hits.options.length = 0;
      form.hitOnologies.options.length = 0;
      labels = new Array();
      conceptIds = new Array();
      preferredNames = new Array();
      ...
      $.ajax({
        ...
        error: function() {
          ...
          //added by teodor
          $('#control_buttons').css("display", "block");
          switchControlsAvailability("active");
          //END added by teodor
        },
        success: function(xml) {
          ...
          if (labels.length > 0) {
            ...
            //added by teodor
            $('#hits').sortOptionsByText();
            $('#hitOnologies').sortOptionsByText();
            $('#bioPortal_1').css("display", "block");
            $('#bioPortal_2').css("display", "block");
            $('#control_buttons').addClass("control_buttons_BP");
            //END added by teodor
          } else {
            form.statusBox.value = 'Sorry but your search did not find any results';
            //added by teodor
            $('#bioPortal_1').css("display", "none");
            $('#bioPortal_2').css("display", "none");
            $('#control_buttons').removeClass("control_buttons_BP");
            //END added by teodor
          }
          //added by teodor
          $('#control_buttons').css("display", "block");
    ...
    function switchControlsAvailability(state)
    {
      if(controlsState != state)
      {
        //don't change to "disabled" if we have ontology results
        if(state == "disable") 
          if(labels.length > 0) return;
        
        controlsState = state;
        $('#control_buttons input').each(function(i, element){
          var auxState = (controlsState == "active" ? "" : "disabled");
          $(element).attr("disabled", auxState);
          if(auxState == "disabled") $(element).addClass("control_buttons_disabled");
          else $(element).removeClass("control_buttons_disabled");
        });
      }
    }
    
    $(document).ready(function(){
      $('#hitOnologies').change(function(){
        filterOntology(document.getElementById("profile_edit_form"));
        $('#hits').sortOptionsByText();
      })
      $('#hitOnologies').attr("multiple","multiple");
      
      //when the keyword changes, only after search can be added
      $('#searchfor').change(function(){
        switchControlsAvailability("disable");
      });
    });
...
<table><tr><td style='vertical-align: top;' nowrap>
...
Enter your search term: <input type='text' name='searchfor' id="searchfor" size='32' maxlength='100' />
<input type='button' value='Search' onclick='return fetchHits(this.form);' /> <img src='/mod/profile_manager/views/default/profile/blank.gif' id='wait' width='31' height='31' alt='' style='border: 0; vertical-align: middle;' />
<input type='text' name='statusBox' size='100' disabled class='noBox' /></p>
<!-- <input type='hidden' name='action' value='' /> -->
<br><br>
</td></tr>
<tr>
<td style='vertical-align: top;'>
  <div id="bioPortal_1" style="display:none">
    Bio-portal response:<br />
    <select name='hits' id="hits" size='12' class='responseSelector' onclick='highlightHit(this.form);'></select>
    Ontology: <input type='text' name='hitLabel' size='50' disabled class='noBox' /><br />
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;URL: <input type='text' name='hitConceptId' size='50' disabled class='noBox' />
  </div>
  <div id="control_buttons" class="" style="display:none">
    <input type='button' value='Add as keyword' title='Click to add entry' onclick='return addKeyHit(this.form);' />
    <input type='button' value='Add as discipline' title='Click to add entry' onclick='return addDisHit(this.form);' />
    <input type='button' value='Add as speciality' title='Click to add entry' onclick='return addSpecHit(this.form);' />
  </div>
  <div id="bioPortal_2" style='text-align: center; vertical-align: top; display:none;'>
    <div id='ontolFilter' style='visibility: hidden;'>
      Filter by ontology (one or many):<br />
      <select name='hitOnologies' id='hitOnologies' size='12' class='mainSelectors' style="width: 400px;"></select><br />
      <input type='button' value='clear all filters' onclick='return removeFilter(this.form);' />
    </div>
  </div>
</td></tr>
...
<td style='width: 32%; text-align: center; vertical-align: top;' nowrap>
  <div class="chosen_boxes">
    Chosen keywords:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' value='x' title='Click to remove entry' onclick='return remKeyHit(this.form);' /><br />
    <select name='pickedKey' id="meducator_subjects" size='12' class='mainSelectors'></select><br />
    <input type='button' value='clear all keywords' onclick='return clearAllKey(this.form);' />
  </div>

  <div class="chosen_boxes">
    Chosen disciplines:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' value='x' title='Click to remove entry' onclick='return remDisHit(this.form);' /><br />
    <select name='pickedDis' size='12' class='mainSelectors'></select><br />
    <input type='button' value='clear all disciplines' onclick='return clearAllDis(this.form);' />
  </div>

  <div class="chosen_boxes">
    Chosen specialities:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='button' value='x' title='Click to remove entry' onclick='return remSpecHit(this.form);' /><br />
    <select name='pickedSpec' size='12' class='mainSelectors'></select><br />
    <input type='button' value='clear all specialties' onclick='return clearAllSpec(this.form);' />
  </div>  
</td>
</tr>
...
<!--NIKOLAS HIDING THIS TO AVOID CLEARING THE WHOLE FORM
<input type='reset' value='reset and search again' onclick='return resetForm(this.form);' /></td></tr>
-->
</table>
<!--      These are the fields for  free text
<p>
