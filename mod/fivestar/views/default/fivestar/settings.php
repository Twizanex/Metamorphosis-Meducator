...
<?php
    ...
    $plugin = find_plugin_settings('fivestar');

    $form = "<br />" . elgg_echo('fivestar:numstars');
    ...
    $form .= elgg_view("input/checkboxes", array(
        'options' => array(elgg_echo('fivestar:settings:change_cancel') => 1), 
        'internalname' => 'change_vote', 
        'value' => $plugin->change_vote )
    ); 
    $form .= "</p>";

    $form .= '<br /><h4>'.elgg_echo('fivestar:settings:view_heading').':</h4><br />';

    $x = 1;
    $lines = explode("\n", get_plugin_setting('view', 'fivestar'));
    ...
    $form .= '<p><a onmouseover="this.style.cursor=\'pointer\';" onClick="addFormField(); return false;"><b>'.elgg_echo('fivestar:settings:add_view').'</b></a></p>';


    $form .= "<br><br>" . elgg_view('input/submit', array('value' => elgg_echo("save")));

	$action = $vars['url'] . 'action/fivestar/settings';
