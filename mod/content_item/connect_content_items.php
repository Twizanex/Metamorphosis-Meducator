<?php
    ...
    // set the title
    $title = "Select the Repurposed(child)   Educational Resource";
...
$AR2 .= "<table class=\"collectionMembersTable\">";
if ($result!=NULL)
...
for($i=0;$i<$total_users;$i++)
{
	...
	$AR2 .= "<td class=\"textarea:focus\" > <form action=\"{$CONFIG->wwwroot}mod/content_item/select_content_items.php\" method=\"post\"> ".$j.") ".$row->username."    <input type=\"hidden\" name=\"cont_it_id\" value=\"{$row->guid}\"/><input type=\"submit\" name=\"Select\" value=\"Select\" /></form></td>";
	//$AR2 .= "<td><a href=\"{$CONFIG->wwwroot}mod/content_item/select_content_items.php\">".$row->name."  ".$row->guid."</a></td>";
