...
$admin = method_exists ( $_SESSION['user'] , "isAdmin" )
         ? $_SESSION['user']->isAdmin()
         : ($_SESSION['user']->admin || $_SESSION['user']->siteadmin);
...
class class_elggMan {
	...
	{
	 return (
	   method_exists ( $_SESSION['user'] , "isAdmin" )
	       ? $_SESSION['user']->isAdmin()
	       : ($_SESSION['user']->admin || $_SESSION['user']->siteadmin)
	 );
	}
  ...
  function method_getAllColumns($params, $error)
	...
	// alle zugeordeneten User liefern
	{
		...
		switch ($request) {
			case "friends":
				switch($friendselect){
					...
					case elgg_echo('ElggMan:friends:incoming'):
						$in_count = westorElggMan_get_entities_from_relationship('friendrequest', $owner->guid, true, "", "", 0, "", 0, 0, true);
						$users = westorElggMan_get_entities_from_relationship("friendrequest", $owner->guid, true, "", "", 0, "", $in_count);
						break;
					case elgg_echo('ElggMan:friends:outgoing'):
						$sent_count = westorElggMan_get_entities_from_relationship("friendrequest", $owner->guid, false, "user", "", 0, "", 0, 0, true);
						$users = westorElggMan_get_entities_from_relationship("friendrequest", $owner->guid, false, "user", "", 0, "", $sent_count);
				} // switch
				break;
			case "contacts":
			...
			default:
				// "users" || "blockedUsers":
				$users = get_entities($type = "user", $subtype = "", $owner_guid = 0, $order_by = "", $limit);
				break;
		} // switch
		...
		if (is_array($users)) {
			foreach($users as $user) {
				...
				// a = admin
				// s = self
				$entry["metadata"][$user->getGUID()] = array('f' => is_object($owner->isFriendsWith($user->guid)),'a' => $user->admin, 's' => $owner->getGUID() == $user->getGUID());
			}
		}
    ...
    $i = 4;
      ...
      $data = $params[0]->data;
		...
		// Message Functions
		private function createMessageObject($subtype, $mType, $from, $to, $subject, $body, $shedule = null){
		...
		public function sendMsgNow($message) {
			$mType = $message->mType;
      $recp = get_entity($message->toId);
			$sender = get_entity($message->fromId);
      ...
      $from = (int) $owner->guid;
			...
			// shedule
			if ($params[4]) { // in Form yyyy-mm-tt hh:mm
				$messageShedule = "'" . mysql_real_escape_string($params[4]) . "'";
			} else {
				$messageShedule = 'now()';
			}
			...
			if (is_array($recipients)) {
				foreach($recipients as $recipient) {
					...
					if ($recp instanceof ElggUser || $recp->getSubtype() == "PrivateContact") {
						...
						$msgObjToSend = $this->createMessageObject("messages", $mType, $from, $to, $subject, $body, $shedule);
		...
		function method_getMessages($params, $error)
		{
          ...
          'metadata_value' => $_SESSION['user']->guid,
					'types' => 'object',
					'subtypes' => 'messages',
					'owner_guid' => $_SESSION['user']->guid));
					} else {
						$messages = get_entities_from_metadata('fromId', $_SESSION['user']->guid, 'object', 'messages', $_SESSION['user']->guid, MAXENTRIES);
					}
					break;
				case elgg_echo("ElggMan:messages:inbox"):
					...
					'owner_guid' => $_SESSION['user']->guid
						...
						$messages = get_entities_from_metadata('toId', $_SESSION['user']->guid, 'object', 'messages', $_SESSION['user']->guid, MAXENTRIES);
					}
					break;
          ...
          'metadata_value' => $_SESSION['user']->guid,
          'metadata_name' => 'mType',
          'metadata_value' => 'HQ-SMS',
          'types' => 'object',
          'subtypes' => 'messages',
          'owner_guid' => $_SESSION['user']->guid
						));
					} else {
						//          $messages = get_entities_from_metadata('toId', $_SESSION['user']->guid, 'object', 'messages', 'owner_guid', $_SESSION['user']->guid, 'mType', 'HQ-SMS', 'limit', MAXENTRIES);
					}
					break;
			}
			...
			if (is_array($messages)) {
                ...
                $message_state = '';
						...
						$message_time = substr(date("c", $message->time_created), 0, 10) . ' ' . substr(date("c", $message->time_created), 11, 8);
		...
		if($params[0] == 'my') {
			...
			if (is_array($groupobjects)) {
				foreach($groupobjects as $group){
					...
					$entry[] = array($group->getGUID(), $group->getIcon('tiny'), $groupname, $groupowner, $group->isPublicMembership());
				}
			}
		} else {
	...
	function method_getGroupMembers($params, $error)
	{
		...
		$group = get_entity($params[0]);
		$members = $group->getMembers(MAXENTRIES);

		$entry = array();
		foreach($members as $member){
			$entry[] = array($member->getGUID(), $member->getIcon('topbar'), $member->name );
		}
